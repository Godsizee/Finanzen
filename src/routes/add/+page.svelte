...
<div class="flex h-full flex-col bg-slate-50 p-4 pt-8 max-[340px]:p-2 max-[340px]:pt-6">
	...
	<form onsubmit={handleSubmit} class="flex flex-1 flex-col gap-6">
		<div class="flex flex-col gap-2">
			...
			<!-- Fast Favorites Grid -->
			{#if type === 'expense'}
				<div class="mt-1 flex gap-2 overflow-x-auto pb-1 text-slate-600 select-none">
					<button
						type="button"
						onclick={() => selectFavorite('Rewe', 'Lebensmittel')}
						class="shrink-0 rounded-full bg-slate-100 px-3 py-1.5 text-xs font-semibold transition-all hover:bg-slate-200 active:scale-95"
					>
						🛒 Rewe
					</button>
					<button
						type="button"
						onclick={() => selectFavorite('Tanken', 'Auto & Transport')}
						class="shrink-0 rounded-full bg-slate-100 px-3 py-1.5 text-xs font-semibold transition-all hover:bg-slate-200 active:scale-95"
					>
						🚗 Tanken
					</button>
					<button
						type="button"
						onclick={() => selectFavorite('dm Drogerie', 'Haushalt')}
						class="shrink-0 rounded-full bg-slate-100 px-3 py-1.5 text-xs font-semibold transition-all hover:bg-slate-200 active:scale-95"
					>
						🧼 dm Drogerie
					</button>
					<button
						type="button"
						onclick={() => selectFavorite('Restaurant', 'Hobby & Freizeit')}
						class="shrink-0 rounded-full bg-slate-100 px-3 py-1.5 text-xs font-semibold transition-all hover:bg-slate-200 active:scale-95"
					>
						🍔 Restaurant
					</button>
				...
				<div class="flex flex-col gap-2">
					...
					<div class="flex gap-2 max-[340px]:gap-1">
						<Button
							...
							class="flex-1 max-[340px]:min-h-[40px] max-[340px]:px-1 max-[340px]:text-xs"
							onclick={() => (payer = 'ich')}
						>
							Ich
						</Button>
